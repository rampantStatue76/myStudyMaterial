...
在创建指令：/directives/permission.js

```javascript
import { useUserStore } from '@/stores/user' // 这里假设你用的是 Pinia
...
export default {
  mounted(el, binding) {
    ...
    // 判断是否有权限
    if (value && Array.isArray(userPermissions)) {
      ...
      console.error('请传入权限标识值，如 v-permission="\'delete\'"')
    }
  },
}
...
<template>
  <div>
    ...
    <button v-permission="'create'">新增</button>
  </div>
</template>
